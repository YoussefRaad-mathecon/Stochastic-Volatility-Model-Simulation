...
def generateHestonPathTGMDisc(S0, v0, r, kappa, theta, sigma, rho, lambda_, T, n):
    S = np.zeros(n + 1)
    S[0] = S0
    v = np.zeros(n + 1)
    v[0] = v0
...
def priceHestonCallViaTGMC(S0, v0, r, kappa, theta, sigma, rho, lambda_, T, n, N, K):
    ...
    for i in range(N):
        S, v_zero_count = generateHestonPathTGMDisc(S0, v0, r, kappa, theta, sigma, rho, lambda_, T, n)
